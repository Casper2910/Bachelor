from cryptography.hazmat.primitives.serialization import load_ssh_private_key, load_ssh_public_key
import multibase
private_key = load_ssh_private_key(
    b"""-----BEGIN OPENSSH PRIVATE KEY-----

-----END OPENSSH PRIVATE KEY-----""",
    password=None,)

signature = private_key.sign(b'did:iota:test1')

print(signature)

# Load the public key
public_key = load_ssh_public_key(
    b'ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAILFPQeUmSdc9JyA0xAzh2iriV4njIbI8EYG1c2VpYUK1')

base58 = multibase.encode('base58btc', b'ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAILFPQeUmSdc9JyA0xAzh2iriV4njIbI8EYG1c2VpYUK1')
print("base58:", base58)
decoded = multibase.decode(base58)
uft8_decoded = decoded.decode('utf-8')
print(decoded)
print(uft8_decoded)

# Verify the signature
try:
    public_key.verify(signature, b'did:iota:test1')
    print("Signature verified successfully.")
except Exception as e:
    print("Verification failed:", e)